...
public class ReportController {
	...
	// 본인 계정 외 삭제 제한 (관리자 계정 삭제권한 추가 병합 시 이것으로 할 것!)
	...
	// 본인 외 수정 제한 -> 병합시 이것으로 (관리자 계정 누락됐었음)
	@GetMapping("/edit/{id}")
	...
	@PostMapping("/update") // 본인 및 관리자 수정 가능 병합시 이 내용으로 할 것
	public String updateReport(@ModelAttribute Report report,
	    ...
	    // 관리자 또는 작성자만 수정 가능
	    if (!(existingReport.getId().equals(loginUser.getUsername()) || loginUser.getRole().equals("ADMIN"))) {
	    ...
	    if (!file.isEmpty()) {
	        try {
	            ...
	            existingReport.addImage(img);        // 새 이미지 추가
	        } catch (IOException e) {
	            e.printStackTrace();
	        }
	    }
